...
const NewsFeedList = () => {
  ...
  useEffect(()=>{
    data.forEach(element => {
      const fetchImages = async () => {
        const sRef = await storageRef(storage, `NewsPhotos/${element.Id}`);
      ...
      const loadImages = async () => {
        const _url = await fetchImages();
        setUrlArray((prevArray) => ({ ...prevArray, [element.Id]:_url }));
      };
        loadImages()
    });
  ...
  const handleDelete = (id) => {
    const dbRef = ref(db, `NewsFeed/${id}`);
    remove(dbRef);
  };
  ...
  const handleEdit = (card) => {
    setModalData({ ...card });
    const fetchImages = async () => {
      const sRef = await storageRef(storage, `NewsPhotos/${card.Id}`);
  ...
  const handleSubmit = () => {
    const { id,Id, Description, LongDescription, Title } = modalData;
      ...
      {data.map((card) => (
        <Card key={card.id} border="gray" style={{ borderWidth: '2px' }}  className=" mb-3">
          <Card.Body>
            ...
            <img src={urlArray[card.Id]} alt='News' style={{width:'100px',height:'100px'}}/>
            ...
            <div style={{ display: 'flex', gap: '10px',verticalAlign:'middle'}}>
